...
export default function Login() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await fetch("/api/dilution/auth", {
        method: "POST",
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center px-4">
      ...
      >
        <h1 className="font-display text-2xl text-cotto-red mb-1">Cotto · Dilution</h1>
        <p className="text-sm text-cotto-red/70 mb-4">Private SAFE dilution calculator.</p>
        <input
          type="password"
          autoFocus
          value={password}
