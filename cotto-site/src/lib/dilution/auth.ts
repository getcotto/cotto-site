...
// Separate gate from /dash — its own secret + cookie so sharing this link
// never exposes the dash password.
export const COOKIE_NAME = "dilution_session";
const COOKIE_MAX_AGE = 60 * 60 * 24 * 60; // 60 days

function getSecret(): string | null {
  return process.env.DILUTION_SECRET || null;
}
...
export function buildSessionCookie(): {
  ...
  const secret = getSecret();
  if (!secret) throw new Error("DILUTION_SECRET not set");
  return {
    name: COOKIE_NAME,
    value: secret,
    options: {
